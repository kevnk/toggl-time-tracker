// Generated by CoffeeScript 1.7.1
(function() {
  var moment, should;

  should = require('should');

  moment = require('../lib/moment-range');

  describe('Moment', function() {
    var dr, m_1, m_2, m_end, m_start;
    dr = moment.range(new Date(Date.UTC(2011, 2, 5), new Date(Date.UTC(2011, 5, 5))));
    m_1 = moment('2011-04-15', 'YYYY-MM-DD');
    m_2 = moment('2012-12-25', 'YYYY-MM-DD');
    m_start = moment('2011-03-05', 'YYYY-MM-DD');
    m_end = moment('2011-06-05', 'YYYY-MM-DD');
    describe('#range()', function() {
      it('should return a DateRange with start & end properties', function() {
        dr = moment.range(m_1, m_2);
        moment.isMoment(dr.start).should.be["true"];
        return moment.isMoment(dr.end).should.be["true"];
      });
      return it('should support string units like `year`, `month`, `week`, `day`, `minute`, `second`, etc...', function() {
        dr = m_1.range('year');
        dr.start.valueOf().should.equal(moment(m_1).startOf('year').valueOf());
        return dr.end.valueOf().should.equal(moment(m_1).endOf('year').valueOf());
      });
    });
    return describe('#within()', function() {
      it('should determine if the current moment is within a given range', function() {
        m_1.within(dr).should.be["true"];
        return m_2.within(dr).should.be["false"];
      });
      return it('should consider the edges to be within the range', function() {
        m_start.within(dr).should.be["true"];
        return m_end.within(dr).should.be["true"];
      });
    });
  });

  describe('DateRange', function() {
    var d_1, d_2, d_3, d_4, m_1, m_2, m_3, m_4, s_end, s_start;
    d_1 = new Date(Date.UTC(2011, 2, 5));
    d_2 = new Date(Date.UTC(2011, 5, 5));
    d_3 = new Date(Date.UTC(2011, 4, 9));
    d_4 = new Date(Date.UTC(1988, 0, 1));
    m_1 = moment.utc('06-05-1996', 'MM-DD-YYYY');
    m_2 = moment.utc('11-05-1996', 'MM-DD-YYYY');
    m_3 = moment.utc('08-12-1996', 'MM-DD-YYYY');
    m_4 = moment.utc('01-01-2012', 'MM-DD-YYYY');
    s_start = '1996-08-12T00:00:00.000Z';
    s_end = '2012-01-01T00:00:00.000Z';
    describe('constructor', function() {
      it('should allow initialization with date string', function() {
        var dr;
        dr = moment.range(s_start, s_end);
        moment.isMoment(dr.start).should.be["true"];
        return moment.isMoment(dr.end).should.be["true"];
      });
      it('should allow initialization with Date object', function() {
        var dr;
        dr = moment.range(d_1, d_2);
        moment.isMoment(dr.start).should.be["true"];
        return moment.isMoment(dr.end).should.be["true"];
      });
      return it('should allow initialization with Moment object', function() {
        var dr;
        dr = moment.range(m_1, m_2);
        moment.isMoment(dr.start).should.be["true"];
        return moment.isMoment(dr.end).should.be["true"];
      });
    });
    describe('#clone()', function() {
      return it('should deep clone range', function() {
        var dr1, dr2;
        dr1 = moment().range(s_start, s_end);
        dr2 = dr1.clone();
        dr2.start.add(2, 'days');
        return dr1.start.toDate().should.not.equal(dr2.start.toDate());
      });
    });
    describe('#by()', function() {
      it('should iterate correctly by range', function() {
        var acc, d1, d2, dr1, dr2;
        acc = [];
        d1 = new Date(Date.UTC(2012, 2, 1));
        d2 = new Date(Date.UTC(2012, 2, 5));
        dr1 = moment.range(d1, d2);
        dr2 = 1000 * 60 * 60 * 24;
        dr1.by(dr2, function(m) {
          return acc.push(m);
        });
        acc.length.should.eql(5);
        acc[0].utc().date().should.eql(1);
        acc[1].utc().date().should.eql(2);
        acc[2].utc().date().should.eql(3);
        acc[3].utc().date().should.eql(4);
        return acc[4].utc().date().should.eql(5);
      });
      it('should iterate correctly by duration', function() {
        var acc, d1, d2, dr1, dr2;
        acc = [];
        d1 = new Date(Date.UTC(2014, 9, 6, 0, 0));
        d2 = new Date(Date.UTC(2014, 9, 6, 23, 59));
        dr1 = moment.range(d1, d2);
        dr2 = moment.duration(15, 'minutes');
        dr1.by(dr2, function(m) {
          return acc.push(m);
        });
        acc.length.should.eql(96);
        acc[0].minute().should.eql(0);
        return acc[95].minute().should.eql(45);
      });
      it('should iterate correctly by shorthand string', function() {
        var acc, d1, d2, dr1, dr2;
        acc = [];
        d1 = new Date(Date.UTC(2012, 2, 1));
        d2 = new Date(Date.UTC(2012, 2, 5));
        dr1 = moment.range(d1, d2);
        dr2 = 'days';
        dr1.by(dr2, function(m) {
          return acc.push(m);
        });
        acc.length.should.eql(5);
        acc[0].utc().date().should.eql(1);
        acc[1].utc().date().should.eql(2);
        acc[2].utc().date().should.eql(3);
        acc[3].utc().date().should.eql(4);
        return acc[4].utc().date().should.eql(5);
      });
      it('should iterate correctly by year over a Date-constructed range when leap years are involved', function() {
        var acc, d1, d2, dr1, dr2;
        acc = [];
        d1 = new Date(Date.UTC(2011, 1, 1));
        d2 = new Date(Date.UTC(2013, 1, 1));
        dr1 = moment.range(d1, d2);
        dr2 = 'years';
        dr1.by(dr2, function(m) {
          return acc.push(m.utc().year());
        });
        return acc.should.eql([2011, 2012, 2013]);
      });
      it('should iterate correctly by year over a moment()-constructed range when leap years are involved', function() {
        var acc, dr1, dr2;
        acc = [];
        dr1 = moment.range(moment('2011', 'YYYY'), moment('2013', 'YYYY'));
        dr2 = 'years';
        dr1.by(dr2, function(m) {
          return acc.push(m.year());
        });
        return acc.should.eql([2011, 2012, 2013]);
      });
      it('should iterate correctly by month over a moment()-constructed range when leap years are involved', function() {
        var acc, dr1, dr2;
        acc = [];
        dr1 = moment.range(moment.utc('2012-01', 'YYYY-MM'), moment.utc('2012-03', 'YYYY-MM'));
        dr2 = 'months';
        dr1.by(dr2, function(m) {
          return acc.push(m.utc().format('YYYY-MM'));
        });
        return acc.should.eql(['2012-01', '2012-02', '2012-03']);
      });
      return it('should iterate correctly by month over a Date-contstructed range when leap years are involved', function() {
        var acc, d1, d2, dr1, dr2;
        acc = [];
        d1 = new Date(Date.UTC(2012, 0));
        d2 = new Date(Date.UTC(2012, 2));
        dr1 = moment.range(d1, d2);
        dr2 = 'months';
        dr1.by(dr2, function(m) {
          return acc.push(m.utc().format('YYYY-MM'));
        });
        return acc.should.eql(['2012-01', '2012-02', '2012-03']);
      });
    });
    describe('#contains()', function() {
      it('should work with Date objects', function() {
        var dr;
        dr = moment.range(d_1, d_2);
        dr.contains(d_3).should.be["true"];
        return dr.contains(d_4).should.be["false"];
      });
      it('should work with Moment objects', function() {
        var dr;
        dr = moment.range(m_1, m_2);
        dr.contains(m_3).should.be["true"];
        return dr.contains(m_4).should.be["false"];
      });
      it('should work with DateRange objects', function() {
        var dr1, dr2;
        dr1 = moment.range(m_1, m_4);
        dr2 = moment.range(m_3, m_2);
        dr1.contains(dr2).should.be["true"];
        return dr2.contains(dr1).should.be["false"];
      });
      return it('should be an inclusive comparison', function() {
        var dr1;
        dr1 = moment.range(m_1, m_4);
        dr1.contains(m_1).should.be["true"];
        dr1.contains(m_4).should.be["true"];
        return dr1.contains(dr1).should.be["true"];
      });
    });
    describe('#overlaps()', function() {
      return it('should work with DateRange objects', function() {
        var dr_1, dr_2, dr_3, dr_4;
        dr_1 = moment.range(m_1, m_2);
        dr_2 = moment.range(m_3, m_4);
        dr_3 = moment.range(m_2, m_4);
        dr_4 = moment.range(m_1, m_3);
        dr_1.overlaps(dr_2).should.be["true"];
        dr_1.overlaps(dr_3).should.be["false"];
        return dr_4.overlaps(dr_3).should.be["false"];
      });
    });
    describe('#intersect()', function() {
      var d_5, d_6, d_7, d_8;
      d_5 = new Date(Date.UTC(2011, 2, 2));
      d_6 = new Date(Date.UTC(2011, 4, 4));
      d_7 = new Date(Date.UTC(2011, 6, 6));
      d_8 = new Date(Date.UTC(2011, 8, 8));
      it('should work with [---{==]---} overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_7);
        dr_2 = moment.range(d_6, d_8);
        return dr_1.intersect(dr_2).isSame(moment.range(d_6, d_7)).should.be["true"];
      });
      it('should work with {---[==}---] overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_6, d_8);
        dr_2 = moment.range(d_5, d_7);
        return dr_1.intersect(dr_2).isSame(moment.range(d_6, d_7)).should.be["true"];
      });
      it('should work with [{===]---} overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_6);
        dr_2 = moment.range(d_5, d_7);
        return dr_1.intersect(dr_2).isSame(moment.range(d_5, d_6)).should.be["true"];
      });
      it('should work with {[===}---] overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_7);
        dr_2 = moment.range(d_5, d_6);
        return dr_1.intersect(dr_2).isSame(moment.range(d_5, d_6)).should.be["true"];
      });
      it('should work with [---{===]} overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_7);
        dr_2 = moment.range(d_6, d_7);
        return dr_1.intersect(dr_2).isSame(moment.range(d_6, d_7)).should.be["true"];
      });
      it('should work with {---[===}] overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_6, d_7);
        dr_2 = moment.range(d_5, d_7);
        return dr_1.intersect(dr_2).isSame(moment.range(d_6, d_7)).should.be["true"];
      });
      it('should work with [---] {---} overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_6);
        dr_2 = moment.range(d_7, d_8);
        return should.strictEqual(dr_1.intersect(dr_2), null);
      });
      it('should work with {---} [---] overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_7, d_8);
        dr_2 = moment.range(d_5, d_6);
        return should.strictEqual(dr_1.intersect(dr_2), null);
      });
      it('should work with [---]{---} overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_6);
        dr_2 = moment.range(d_6, d_7);
        return should.strictEqual(dr_1.intersect(dr_2), null);
      });
      it('should work with {---}[---] overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_6, d_7);
        dr_2 = moment.range(d_5, d_6);
        return should.strictEqual(dr_1.intersect(dr_2), null);
      });
      it('should work with {--[===]--} overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_6, d_7);
        dr_2 = moment.range(d_5, d_8);
        return dr_1.intersect(dr_2).isSame(dr_1).should.be["true"];
      });
      it('should work with [--{===}--] overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_8);
        dr_2 = moment.range(d_6, d_7);
        return dr_1.intersect(dr_2).isSame(dr_2).should.be["true"];
      });
      it('should work with [{===}] overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_6);
        dr_2 = moment.range(d_5, d_6);
        return dr_1.intersect(dr_2).isSame(dr_2).should.be["true"];
      });
      return it('should work with [--{}--] overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_6, d_6);
        dr_2 = moment.range(d_5, d_7);
        return dr_1.intersect(dr_2).isSame(dr_1).should.be["true"];
      });
    });
    describe('#add()', function() {
      var d_5, d_6, d_7, d_8;
      d_5 = new Date(Date.UTC(2011, 2, 2));
      d_6 = new Date(Date.UTC(2011, 4, 4));
      d_7 = new Date(Date.UTC(2011, 6, 6));
      d_8 = new Date(Date.UTC(2011, 8, 8));
      it('should add ranges with [---{==]---} overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_7);
        dr_2 = moment.range(d_6, d_8);
        return dr_1.add(dr_2).isSame(moment.range(d_5, d_8)).should.be["true"];
      });
      it('should add ranges with {---[==}---] overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_6, d_8);
        dr_2 = moment.range(d_5, d_7);
        return dr_1.add(dr_2).isSame(moment.range(d_5, d_8)).should.be["true"];
      });
      it('should add ranges with [{===]---} overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_6);
        dr_2 = moment.range(d_5, d_7);
        return dr_1.add(dr_2).isSame(moment.range(d_5, d_7)).should.be["true"];
      });
      it('should add ranges with {[===}---] overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_7);
        dr_2 = moment.range(d_5, d_6);
        return dr_1.add(dr_2).isSame(moment.range(d_5, d_7)).should.be["true"];
      });
      it('should add ranges with [---{===]} overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_7);
        dr_2 = moment.range(d_6, d_7);
        return dr_1.add(dr_2).isSame(moment.range(d_5, d_7)).should.be["true"];
      });
      it('should add ranges with {---[===}] overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_6, d_7);
        dr_2 = moment.range(d_5, d_7);
        return dr_1.add(dr_2).isSame(moment.range(d_5, d_7)).should.be["true"];
      });
      it('should not add ranges with [---] {---} overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_6);
        dr_2 = moment.range(d_7, d_8);
        return should.strictEqual(dr_1.add(dr_2), null);
      });
      it('should not add ranges with {---} [---] overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_7, d_8);
        dr_2 = moment.range(d_5, d_6);
        return should.strictEqual(dr_1.add(dr_2), null);
      });
      it('should not add ranges with [---]{---} overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_6);
        dr_2 = moment.range(d_6, d_7);
        return should.strictEqual(dr_1.add(dr_2), null);
      });
      it('should not add ranges with {---}[---] overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_6, d_7);
        dr_2 = moment.range(d_5, d_6);
        return should.strictEqual(dr_1.add(dr_2), null);
      });
      it('should add ranges {--[===]--} overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_6, d_7);
        dr_2 = moment.range(d_5, d_8);
        return dr_1.add(dr_2).isSame(moment.range(d_5, d_8)).should.be["true"];
      });
      it('should add ranges [--{===}--] overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_8);
        dr_2 = moment.range(d_6, d_7);
        return dr_1.add(dr_2).isSame(moment.range(d_5, d_8)).should.be["true"];
      });
      return it('should add ranges [{===}] overlaps where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_6);
        dr_2 = moment.range(d_5, d_6);
        return dr_1.add(dr_2).isSame(moment.range(d_5, d_6)).should.be["true"];
      });
    });
    describe('#subtract()', function() {
      var d_5, d_6, d_7, d_8;
      d_5 = new Date(Date.UTC(2011, 2, 2));
      d_6 = new Date(Date.UTC(2011, 4, 4));
      d_7 = new Date(Date.UTC(2011, 6, 6));
      d_8 = new Date(Date.UTC(2011, 8, 8));
      it('should turn [--{==}--] into (--)  (--) where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_8);
        dr_2 = moment.range(d_6, d_7);
        return dr_1.subtract(dr_2).should.eql([moment.range(d_5, d_6), moment.range(d_7, d_8)]);
      });
      it('should turn {--[==]--} into () where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_6, d_7);
        dr_2 = moment.range(d_5, d_8);
        return dr_1.subtract(dr_2).should.eql([]);
      });
      it('should turn {[==]} into () where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_6);
        dr_2 = moment.range(d_5, d_6);
        return dr_1.subtract(dr_2).should.eql([]);
      });
      it('should turn [--{==]--} into (--) where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_7);
        dr_2 = moment.range(d_6, d_8);
        return dr_1.subtract(dr_2).should.eql([moment.range(d_5, d_6)]);
      });
      it('should turn [--{==]} into (--) where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_7);
        dr_2 = moment.range(d_6, d_7);
        return dr_1.subtract(dr_2).should.eql([moment.range(d_5, d_6)]);
      });
      it('should turn {--[==}--] into (--) where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_6, d_8);
        dr_2 = moment.range(d_5, d_7);
        return dr_1.subtract(dr_2).should.eql([moment.range(d_7, d_8)]);
      });
      it('should turn {[==}--] into (--) where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_6, d_8);
        dr_2 = moment.range(d_6, d_7);
        return dr_1.subtract(dr_2).should.eql([moment.range(d_7, d_8)]);
      });
      it('should turn [--] {--} into (--) where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_5, d_6);
        dr_2 = moment.range(d_7, d_8);
        return dr_1.subtract(dr_2).should.eql([dr_1]);
      });
      return it('should turn {--} [--] into (--) where (a=[], b={})', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_7, d_8);
        dr_2 = moment.range(d_5, d_6);
        return dr_1.subtract(dr_2).should.eql([dr_1]);
      });
    });
    describe('#isSame()', function() {
      it('should return true if the start and end of both DateRange objects equal', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_1, d_2);
        dr_2 = moment.range(d_1, d_2);
        return dr_1.isSame(dr_2).should.be["true"];
      });
      it('should return false if the starts differ between objects', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_1, d_3);
        dr_2 = moment.range(d_2, d_3);
        return dr_1.isSame(dr_2).should.be["false"];
      });
      return it('should return false if the ends differ between objects', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_1, d_2);
        dr_2 = moment.range(d_1, d_3);
        return dr_1.isSame(dr_2).should.be["false"];
      });
    });
    describe('#valueOf()', function() {
      it('should be the value of the range in milliseconds', function() {
        var dr;
        dr = moment.range(d_1, d_2);
        return dr.valueOf().should.eql(d_2.getTime() - d_1.getTime());
      });
      return it('should correctly coerce to a number', function() {
        var dr_1, dr_2;
        dr_1 = moment.range(d_4, d_2);
        dr_2 = moment.range(d_3, d_2);
        return (dr_1 > dr_2).should.be["true"];
      });
    });
    describe('#toDate()', function() {
      return it('should be return a array like [dateObject, dateObject]', function() {
        var dr, dr_todate;
        dr = moment.range(d_1, d_2);
        dr_todate = dr.toDate();
        dr_todate.length.should.eql(2);
        dr_todate[0].valueOf().should.eql(d_1.valueOf());
        return dr_todate[1].valueOf().should.eql(d_2.valueOf());
      });
    });
    describe('#diff()', function() {
      return it('should use momentjs’ diff method', function() {
        var dr;
        dr = moment.range(d_1, d_2);
        dr.diff('months').should.equal(3);
        dr.diff('days').should.equal(92);
        return dr.diff().should.equal(7948800000);
      });
    });
    return describe('#center()', function() {
      return it('should use momentjs’ center method', function() {
        var dr;
        d_1 = new Date(Date.UTC(2011, 2, 5));
        d_2 = new Date(Date.UTC(2011, 3, 5));
        dr = moment.range(d_1, d_2);
        return dr.center().valueOf().should.equal(1300622400000);
      });
    });
  });

}).call(this);
